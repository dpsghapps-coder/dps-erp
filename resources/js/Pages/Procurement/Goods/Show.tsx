...
export default function GoodsShow() {
    ...
    const supplierPrices = good.supplier_prices || [];
    ...
    const handleAdd = (e: React.FormEvent) => {
        e.preventDefault();
        post(`/procurement/goods/${good.id}/suppliers`, {
            onSuccess: () => {
    ...
    const openEdit = () => {
        setEditData({ ...good });
        setEditAttributeValues(good.attributes || {});
        setEditPicture(null);
    ...
    return (
        <AppLayout>
            <Head title={good.item_name} />

            <div className="mb-6">
                <Link href="/procurement/goods" className="inline-flex items-center gap-2 text-slate-500 hover:text-slate-700 transition-colors">
                    <ArrowLeft className="w-4 h-4" /> Back to Goods
                </Link>
            </div>
                ...
                <div className="flex gap-6">
                    <div className="flex-1">
                        ...
                        <div className="space-y-4">
                            <div>
                                <p className="text-sm text-slate-500 mb-1">Good ID</p>
                                <p className="font-mono">{good.material_id}</p>
                            </div>
                            <div>
                                <p className="text-sm text-slate-500 mb-1">Name</p>
                                <p className="font-medium">{good.item_name}</p>
                            </div>
                            {good.item_description && (
                                <div>
                                    <p className="text-sm text-slate-500 mb-1">Description</p>
                                    <p className="text-slate-600">{good.item_description}</p>
                                </div>
                            )}
                            ...
                            {good.attributes && Object.keys(good.attributes).length > 0 && (
                                <div>
                                    ...
                                    <div className="grid grid-cols-2 sm:grid-cols-3 gap-2">
                                        {Object.entries(good.attributes).map(([key, value]) => (
                    ...
                    {good.picture && (
                        <div className="shrink-0">
                            <img src={'/storage/' + good.picture} alt={good.item_name} className="w-40 h-40 object-cover rounded-lg border border-slate-200" />
                        </div>
                    )}
                </div>
            </GlassCard>
            ...
            <GlassCard>
                ...
                {supplierPrices.length > 0 ? (
                    ...
                    <div className="text-center py-8">
                        ...
                        <p className="text-slate-500 text-sm mb-4">No suppliers linked to this good</p>
            ...
            {deleteTarget && (
                <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50">
                    <div className="bg-white rounded-2xl p-6 w-full max-w-sm">
                        ...
                        <p className="text-sm text-slate-600 mb-6">Remove this supplier from the good? The supplier record itself will not be deleted.</p>
                        <div className="flex gap-3">
                            <button onClick={() => setDeleteTarget(null)} className="flex-1 glass-button-secondary">Cancel</button>
                            <button onClick={() => { router.delete(`/procurement/goods/suppliers/${deleteTarget}`); setDeleteTarget(null); }} className="flex-1 glass-button bg-red-600 hover:bg-red-700">Remove</button>
                        </div>
                    </div>
                </div>
            )}
