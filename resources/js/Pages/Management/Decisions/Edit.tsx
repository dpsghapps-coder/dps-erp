...
export default function DecisionEdit({ decision, categories, employees }: Props) {
    ...
    const handleSubmit = (e: React.FormEvent) => {
        e.preventDefault();
        put(`/management/decisions/${decision.id}`);
    };

    return (
        <AppLayout>
            <Head title="Edit Decision" />

            <PageHeader
                title="Edit Decision"
                subtitle={decision.number}
                action={
                    <Link href={`/management/decisions/${decision.id}`} className="glass-button-secondary flex items-center gap-2">
