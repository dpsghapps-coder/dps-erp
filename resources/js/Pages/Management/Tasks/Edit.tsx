...
export default function TaskEdit() {
    const { task, employees } = usePage().props as any;
    const { data, setData, put, processing, errors } = useForm({
        ...
        assignee_ids: task.assignees?.map((a: any) => a.id) || [] as number[],
    });
    ...
    const removeAssignee = (id: number) => {
        setData('assignee_ids', data.assignee_ids.filter((a: number) => a !== id));
    };

    const handleSubmit = (e: React.FormEvent) => {
        e.preventDefault();
        put(`/management/tasks/${task.id}`);
    };
    ...
    return (
        <AppLayout>
            ...
            <div className="mb-6">
                <Link href={`/management/tasks/${task.id}`} className="inline-flex items-center gap-2 text-slate-500 dark:text-slate-400 hover:text-slate-700 transition-colors">
                ...
                <GlassCard>
                    <form onSubmit={handleSubmit} className="space-y-4">
                        ...
                        <div>
                            ...
                            {data.assignee_ids.length > 0 && (
                                <div className="flex flex-wrap gap-2 mb-2">
                                    {data.assignee_ids.map((id: number) => {
                        ...
                        <div className="flex gap-3 pt-4 border-t border-slate-200 dark:border-white/10">
                            ...
                            <Link href={`/management/tasks/${task.id}`} className="glass-button-secondary">
                                Cancel
                            </Link>
                        </div>
                    </form>
