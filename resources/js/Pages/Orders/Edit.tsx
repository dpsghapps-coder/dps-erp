...
export default function OrderEdit() {
    ...
    return (
        <AppLayout>
            ...
            <div className="mb-6">
                <Link href={`/orders/${order.id}`} className="inline-flex items-center gap-2 text-slate-400 hover:text-slate-900 dark:hover:text-white transition-colors">
                    <ArrowLeft className="w-4 h-4" /> Back to Order
                </Link>
            </div>

            <PageHeader title={`Edit ${order.order_number}`} subtitle="Update order details and line items" />
                        ...
                        <GlassCard>
                            ...
                            <div className="grid md:grid-cols-3 gap-4">
                                <div>
                                    ...
                                    <p className="glass-input w-full flex items-center text-slate-500">{order.client?.company_name}</p>
                                </div>
                                ...
                                <div>
                                    ...
                                    <select
                                        ...
                                        disabled={!order.client?.contacts?.length}
                                        className="glass-input w-full"
                                    >
                                        <option value="">
                                            {order.client?.contacts?.length ? 'Select Contact' : 'No contacts for this client'}
                                        </option>
                                        {(order.client?.contacts || []).map((ct: any) => (
