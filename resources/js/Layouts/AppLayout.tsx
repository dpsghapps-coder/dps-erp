...
export default function AppLayout({ children }: PropsWithChildren) {
    ...
    // Section collapse only applies once the sidebar itself is expanded --
    // in icon-only rail mode there are no labels to click, so always show
    // the icons regardless of a section's remembered collapsed state.
    const isSectionExpanded = (key: string) => !sidebarOpen || !collapsedSections[key];
        ...
        <div className="min-h-screen bg-slate-100 dark:bg-slate-900">
                ...
                <div className="lg:hidden fixed inset-0 z-[80] bg-black/50" onClick={() => setMobileMenuOpen(false)}>
                    ...
                    >
                        ...
                        <nav className="flex-1 overflow-y-auto scrollbar-thin p-4 pb-24 space-y-1">
                            ...
                            {(hasModulePermission('products') || hasModulePermission('crm') || hasModulePermission('marketing') || hasModulePermission('orders')) && (
            ...
            <aside className={`hidden lg:flex flex-col fixed left-0 top-0 bottom-0 bg-white dark:bg-[#13161f] border-r border-slate-200 dark:border-white/[0.06] transition-all duration-200 ${
                ...
                <nav className="flex-1 overflow-y-auto scrollbar-thin py-4">
                    ...
                    {(hasModulePermission('products') || hasModulePermission('crm') || hasModulePermission('marketing') || hasModulePermission('orders')) && <div className="px-3 mt-4 mb-2">
                        {sidebarOpen && (
                            <button
                                onClick={() => toggleSection('sales')}
                                className="w-full flex items-center justify-between text-xs text-slate-400 uppercase font-medium hover:text-slate-600 dark:hover:text-slate-300 transition-colors"
                        ...
                        <>
                            ...
                            {hasModulePermission('products') && <div className="px-3 mt-1">
                                ...
                                >
                                    ...
                                    {sidebarOpen && <span>Proforma & Proposals</span>}
                                </Link>
                            </div>}
                        </>
                    )}

                    {/* OPERATIONS Section - Production & Reports */}
                        ...
                        <div className="bg-white rounded-t-3xl shadow-2xl overflow-hidden max-h-[80vh] flex flex-col">
                            ...
                            <div className="flex-1 overflow-y-auto p-4 space-y-2">
                                ...
                                >
                                    ...
                                    <span className="font-medium">Proforma & Proposals</span>
                                </Link>
                            </div>
                            <div className="pb-8"></div>
                        </div>
